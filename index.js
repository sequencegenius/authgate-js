...
export class AuthGateClient {
    ...
    /**
     * Verify the session by sending a request to the authentication gate.
     *
     * @return {Promise<string>} The user ID if the session is verified or AuthGateResponse.UNAUTHORIZED if the session is not verified.
     */
    async verifySession() {
        const res = await fetch(`${this.authGateUrl}/verify-session`);
        if (res.status !== 200) {
            return AuthGateResponse.UNAUTHORIZED;
        }
